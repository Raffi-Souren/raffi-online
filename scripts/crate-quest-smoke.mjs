...
try {
  for (const [name, width, height, touch] of [
    ...
    await page.goto(base + "/?app=world&auto=1&debug=1&seed=FIXED&tier=low", { waitUntil: "domcontentloaded" })
    ...
    await page.waitForFunction((el) => el.dataset.playerY !== undefined && Number.isFinite(Number(el.dataset.playerY)), await canvas.elementHandle())
    ...
    for (let attempt = 0; attempt < 6 && (await position()).y > 14.3; attempt++) await step("ArrowUp", 800)
    const against = await position()
    assert.ok(against.y >= 14.06 - 1 / 48 && against.y < 14.3, "held input must reach and stop at Mara: " + JSON.stringify(against))
    ...
    if (name === "desktop") {
      ...
      const visit = async (id) => {
        ...
        if (id !== "mara") {
          await quest.getByRole("region", { name: "Collector dialogue", exact: true }).waitFor()
          if (["warmup-bin", "peak-bin", "yard-bin"].includes(id)) {
            const sleeve = quest.getByRole("img", { name: /record sleeve/ })
            await sleeve.waitFor()
            await sleeve.evaluate((image) => image.decode())
            assert.equal(await sleeve.evaluate((image) => image.complete && image.naturalWidth > 0), true)
            await page.screenshot({ path: out + "/" + id + "-sleeve.png" })
          }
          await quest.getByRole("button", { name: "Continue", exact: true }).first().click()
        }
      }
      await visit("warmup-bin")
      const boothPosition = await position()
      await quest.getByRole("button", { name: "DJ booth", exact: true }).click()
      const booth = quest.getByRole("dialog", { name: "Ghost Deck DJ station", exact: true })
      await booth.waitFor()
      const dj = page.frameLocator('iframe[title="Ghost Deck — real two-deck DJ demo"]')
      await dj.getByRole("button", { name: "Load a demo session", exact: true }).click({ timeout: 45000 })
      await dj.getByRole("button", { name: /Play/ }).first().click()
      await page.waitForTimeout(900)
      assert.deepEqual(await position(), boothPosition, "the hunt moved while Ghost Deck owned input")
      await page.screenshot({ path: out + "/desktop-ghost-deck.png" })
      await page.setViewportSize({ width: 390, height: 844 })
      const returnButton = booth.getByRole("button", { name: "Back to the records", exact: true })
      const returnBounds = await returnButton.boundingBox()
      assert.ok(returnBounds && returnBounds.y >= 0 && returnBounds.y + returnBounds.height <= 844)
      assert.equal(await dj.locator("body").evaluate((body) => body.scrollWidth <= innerWidth + 1), true, "Ghost Deck has horizontal overflow on phone")
      await page.screenshot({ path: out + "/phone-ghost-deck.png" })
      await returnButton.click()
      await page.locator('iframe[title="Ghost Deck — real two-deck DJ demo"]').waitFor({ state: "detached" })
      assert.deepEqual(await position(), boothPosition, "leaving the DJ booth lost the hunt position")
      await page.setViewportSize({ width: 1280, height: 900 })
      pass("Original Ghost Deck loads and plays its real demo, hunt freezes, mobile return stays visible, leaving removes the audio iframe")
